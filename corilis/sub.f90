...
!vu = 0.25*(v(j,k)+v(j,k+1)+v(j-1,k)+v(j-1,k+1))
vu = 0.125*(v(j,k)+v(j,k+1)+v(j-1,k)+v(j-1,k+1)+vstar(j,k)+vstar(j,k+1)+vstar(j-1,k)+vstar(j-1,k+1))
corx = dt*f*vu
du(j,k) = (ustar(j,k)-beta*u(j,k)+corx)/(1.0+beta)-u(j,k)

!uv = 0.25*(u(j,k)+u(j+1,k)+u(j,k-1)+u(j+1,k-1))
uv = 0.125*(u(j,k)+u(j,k+1)+u(j-1,k)+u(j-1,k+1)+ustar(j,k)+ustar(j,k+1)+ustar(j-1,k)+ustar(j-1,k+1))
cory = -dt*f*uv
...
IF(mmode == 4)THEN
  ...
  IF(r2 > 0.0)THEN
    ...
    ELSE
      IF(cfl > 0.0) psi = MIN(2.0*r2/cfl,1.0)
    END IF
  END IF
END IF
